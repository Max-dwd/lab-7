describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let count = await page.$eval('#cart-count', el => parseInt(el.innerText));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    const prods = page.$('product-item');
    for (let i = 0; i < prods.length; i ++ ) {
      const shadowRoot = await products[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      const btnText = await button.getProperty('innerText').jsonValue();
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < prods.length; i++) {
      const shadowRoot = await prods[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      await button.click();
    }
    const count = await page.$eval('#cart-count', el => parseInt(el.innerText));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let product of products) {
      ...
      const btnText = await (await (await shadowRoot.$('button')).getProperty('innerText')).jsonValue();
